...
async def dispatch_board_sighting_pushes(installation_id: str, sighting_id: str) -> None:
    client = _apns_client
    if client is None:
        logger.error(
            "ember_board_sighting_push_skipped",
            extra=log_fields(sighting_id=sighting_id, reason="apns_unavailable"),
        )
        return
    with connect() as conn:
        sighting = conn.execute(
            """
            SELECT id, board_name, board_suffix, received_at
            FROM ember_board_sightings
            WHERE id = ? AND installation_id = ? AND notification_queued = 1
            """,
            (sighting_id, installation_id),
        ).fetchone()
        tokens = conn.execute(
            """
            SELECT t.* FROM ember_member_push_tokens t
    ...
    payload = {
        ...
        "boardName": sighting["board_name"],
        "boardSuffix": sighting["board_suffix"],
    }
    expiration = int((_parse_iso(sighting["received_at"]) + timedelta(hours=24)).timestamp())
    for row in tokens:
        try:
            device_token = _decrypt_token(row["token_ciphertext"])
        except RuntimeError:
            logger.exception(
                "ember_board_sighting_push_token_unavailable",
                extra=log_fields(
        ...
        try:
            result = await client.send(
                ApnsSend(
                    ...
                    environment=row["environment"],
                    payload=payload,
                    collapse_id=f"board-{sighting['board_suffix']}",
                    expiration=expiration,
                )
            )
            latency_ms = int((time.monotonic() - started) * 1000)
            ...
            if result.invalid_token:
                with connect() as conn:
                    ...
                    conn.commit()
            logger.info(
                "ember_board_sighting_push_result",
                extra=log_fields(
                    sighting_id=sighting_id,
                    member_id=row["member_id"],
                    success=result.success,
            ...
            logger.exception(
                "ember_board_sighting_push_failed",
                extra=log_fields(
                    sighting_id=sighting_id,
                    member_id=row["member_id"],
                    device_token=redact_token(device_token),
