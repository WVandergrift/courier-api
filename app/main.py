...
async def ember_home_landing() -> HTMLResponse:
    return HTMLResponse(
        (
            ...
            "background-size:auto,32px 32px,32px 32px;font:16px/1.6 system-ui,-apple-system,sans-serif;overflow-x:hidden}"
            "main{position:relative;z-index:2;width:min(720px,100%)}.brand{color:var(--ember);font-size:13px;font-weight:800;letter-spacing:.16em}"
            ...
            ".house-lights{position:fixed;z-index:1;right:-52px;bottom:24px;width:min(64vw,760px);height:min(44vw,510px);"
            "pointer-events:none;opacity:.7}.strip{position:absolute;height:7px;color:var(--ember);transform-origin:left center;"
            "background-image:radial-gradient(circle,currentColor 0 2.3px,transparent 2.8px);background-size:16px 7px;"
            "filter:drop-shadow(0 0 5px currentColor);animation:roof-chase 5.6s linear infinite}"
            ".roof-a{left:4%;top:52%;width:38%;transform:rotate(-38deg)}.roof-b{left:34%;top:21%;width:46%;transform:rotate(32deg);animation-delay:-1.2s}"
            ".eave{left:3%;top:53%;width:78%;animation-delay:-2.5s}.gable-a{left:47%;top:67%;width:22%;transform:rotate(-43deg);animation-delay:-3.7s}"
            ".gable-b{left:63%;top:51%;width:23%;transform:rotate(43deg);animation-delay:-4.5s}.lower{left:45%;top:68%;width:43%;animation-delay:-1.8s}"
            "@keyframes roof-chase{0%{background-position:0 0;color:#e7562e;opacity:.48}32%{color:#d8a33c;opacity:.95}"
            "64%{color:#3e9c76;opacity:.72}100%{background-position:64px 0;color:#e7562e;opacity:.48}}"
            "@media(max-width:760px){.house-lights{right:-220px;bottom:18px;width:680px;height:440px;opacity:.34}footer{margin-top:54px}}"
            "@media(prefers-reduced-motion:reduce){.strip{animation:none;color:#d8a33c;opacity:.62}}"
            "</style></head><body><div class=\"house-lights\" aria-hidden=\"true\">"
            "<i class=\"strip roof-a\"></i><i class=\"strip roof-b\"></i><i class=\"strip eave\"></i>"
            "<i class=\"strip gable-a\"></i><i class=\"strip gable-b\"></i><i class=\"strip lower\"></i></div>"
            "<main><div class=\"brand\">EMBER</div>"
            "<h1>Light belongs at home.</h1><p>Set up and control compatible architectural lighting "
            ...
            "<footer>Simple for consumers. Open for developers.</footer></main></body></html>"
        ),
